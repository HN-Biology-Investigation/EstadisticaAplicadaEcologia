...
Material de lectura: [Estimating total species richness: Fitting rarefaction by asymptotic approximation](https://doi.org/10.1002/ecs2.4363)
...
*Cuando contamos directamente el número de especies que detectamos en un muestreo, obtenemos lo que llamamos riqueza de especies ingenua o cruda (naïve species richness). Se le llama "ingenua" porque asume implícitamente que nuestro esfuerzo de muestreo fue suficiente para detectar todas las especies presentes, ignorando aquellas que no capturamos u observamos. Este valor es simplemente el conteo de especies distintas registradas, sin ningún tipo de corrección estadística. El problema con esta aproximación es que, especialmente cuando tenemos pocas réplicas (como solo 3 muestras), es casi seguro que subestimamos la riqueza real, ya que muchas especies raras o de baja detectabilidad quedan fuera de nuestra muestra. Por ello, en ecología se recomienda complementar o reemplazar la riqueza ingenua con estimadores más robustos (como Chao2 o Jackknife) cuando el esfuerzo de muestreo es limitado.*
...
Comencemos con una comparación visual utilizando un gráfico de caja.
...
Al aplicar `specpool()` obtenemos varios estimadores de la riqueza total de especies. S es la riqueza observada o naïve (el número de especies que efectivamente detectaste). Chao es un estimador basado en especies raras (las que aparecen en solo una o dos réplicas); es especialmente útil cuando hay muchas especies poco frecuentes. Jackknife 1 y Jackknife 2 se basan en la frecuencia de especies raras pero con distintos órdenes de corrección; Jackknife 2 suele dar estimaciones más altas que Jackknife 1. Bootstrap es otro estimador que tiende a ser más conservador (valores más bajos).
...
La función `estimateR` del paquete vegan calcula estimadores de riqueza basados en datos de abundancia (número de individuos por especie), no solo presencia/ausencia. Los resultados incluyen Chao (estimador de Chao1); se.chao es el error estándar asociado a esa estimación, útil para calcular intervalos de confianza. S.ACE (Abundance-based Coverage Estimator): El Estimador de Cobertura Basado en Abundancia (ACE) es un método no paramétrico utilizado para estimar la riqueza total de especies en una comunidad a partir de datos de abundancia, típicamente en ecología. Estima el número de especies no observadas enfocándose en las especies raras para calcular la cobertura de la muestra, lo que lo hace más robusto que la simple riqueza observada. se.ACE es su error estándar.
...
Utilice la base de datos `bird.sta` para estimar la riqueza de especies. Compare la riqueza naïve (observada) versus la riqueza estimada mediante un gráfico de caja que muestre los intervalos de confianza al 95% para cada categoría de la variable `basin`. Complemente el análisis con una curva de rarefacción que permita evaluar la integridad del muestreo en función del esfuerzo.
